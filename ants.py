...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while next_place != hive:
            if next_place.bees and counter >= self.min_range and counter <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            copied_list = list(self.place.bees)
            for bee in copied_list:
                bee.reduce_armor(self.damage)
            for bee in self.place.bees:
                if bee not in copied_list:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 9
        self.list_of_doubled_ants = []
        QueenAnt.queen_ant_count += 1
        if QueenAnt.queen_ant_count > 1:
            self.imposter = True
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while next_place != None:
                if next_place.ant:
                    if next_place.ant not in self.list_of_doubled_ants:
                        next_place.ant.damage *= 2
                        if next_place.ant.container and next_place.ant.ant and next_place.ant.ant not in self.list_of_doubled_ants:
                            next_place.ant.ant.damage *= 2
                            self.list_of_doubled_ants.append(next_place.ant.ant)
                        self.list_of_doubled_ants.append(next_place.ant)
                    else:
                        if next_place.ant.container and next_place.ant.ant and next_place.ant.ant not in self.list_of_doubled_ants:
                            next_place.ant.ant.damage *= 2
                            self.list_of_doubled_ants.append(next_place.ant.ant)
...
def make_stun(action):
    ...
    # BEGIN Problem EC
    def effect(timer):
        def stunner(colony):   # cash muney records
            nonlocal timer
            if timer > 0:
                timer -= 1
            else:
                return action(colony)
        return stunner     # pop bottles
    return effect
    # END Problem EC
